...
import { getPending, updatePending, deletePending, confirmBatch } from '../api/upload'
...
export default function UploadPreview() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [pendingRes, budgetRes] = await Promise.all([getPending(batchId), getBudget()])
  ...
  async function handleAddCategory(rowId) {
    ...
    const id = newCategoryName.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '')
    const color = '#' + Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0')
    const newCategory = { id, name: newCategoryName.trim(), monthly_target: 0, color }
    const b = await getBudget()
    await updateBudget({ categories: [...b.categories, newCategory], income: b.income })
  ...
  if (!batchId) {
    return <p className="text-sm text-ink-500">No upload batch specified. Try uploading a CSV again.</p>
  }
  if (loading) {
    return <p className="text-sm text-ink-500">Loading uploaded rows…</p>
  }
  if (!items.length) {
    return <p className="text-sm text-ink-500">Nothing left to review in this batch — it may already be confirmed.</p>
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl text-ink-900">Review Upload</h1>
      ...
      <div className="bg-surface border border-line rounded-xl2 shadow-card divide-y divide-line">
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={`p-4 flex flex-wrap items-center gap-3 ${flagged ? 'bg-over/5' : ''}`}>
              <div className="w-24 shrink-0 text-sm text-ink-500 tabular">{item.date}</div>
              <div className="flex-1 min-w-[160px]">
                <p className="text-sm font-medium text-ink-900">{item.source}</p>
                <input
                  placeholder="Add nickname…"
                  defaultValue={item.nickname || ''}
                  onBlur={(e) => e.target.value !== (item.nickname || '') && handleField(item.id, { nickname: e.target.value })}
              ...
              {addingCategoryFor === item.id ? (
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleAddCategory(item.id)}
                    className="text-sm rounded-md border border-line px-2 py-1 w-40"
                  />
                  <button onClick={() => handleAddCategory(item.id)} className="text-xs font-semibold text-accent">
                    Add
                  </button>
                  <button onClick={() => setAddingCategoryFor(null)} className="text-xs text-ink-500">
                    Cancel
                  </button>
                </div>
              ) : (
                <select
                  ...
                  onChange={(e) => {
                    if (e.target.value === '__new__') {
                      ...
                      handleField(item.id, { category_id: e.target.value || null })
                    }
                  }}
                  className={`text-sm rounded-md border px-2.5 py-1.5 w-44 ${
                    flagged ? 'border-over' : item.category_id ? 'border-line' : 'border-warn/60 bg-warn/5'
                  }`}
                >
              ...
              <button
                onClick={() => handleField(item.id, { recurring: !item.recurring })}
              ...
              <span className={`text-sm font-medium tabular w-24 text-right ${item.amount < 0 ? 'text-ink-900' : 'text-good'}`}>
                {currency(item.amount)}
              </span>

              <button onClick={() => handleDelete(item.id)} className="p-2 rounded-md text-over hover:bg-over/10">
                <Trash2 size={15} />
              </button>
            </div>
          )

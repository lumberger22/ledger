...
**[DONE, with a deploy-time caveat]** `Dockerfile` now creates and switches to a non-root `appuser` after installing dependencies. **Check `server-deploy.sh` on the EC2 host**: if `/data` is a bind-mounted host directory, the container's `chown` only affects the image layer, not the host-side ownership -- the host `/data` directory may need `chown` to match `appuser`'s UID (or the `docker run`/compose config needs a matching `--user` flag) or the app will fail to write the SQLite DB after this ships. Verify with `docker exec <container> id appuser` and `ls -ln` on the host path before/after the first deploy with this change. Dependency scanning (`pip-audit`/`npm audit`) is still not wired in -- left as a manual/CI follow-up.
